                ...
                <div class="single-post-wrap">
                    ...
                    <div class="details">
                        <div class="post-meta-single mb-4 pt-1">
                            <ul class="d-flex flex-wrap" style="gap: 10px; list-style: none; padding: 0;">
                                @if($article->category)
                                <li><a href="{{ route('category.show', $article->category->slug) }}" class="tag-base tag-blue" style="margin-right: 0;">{{ $article->category->name }}</a></li>
                                @endif
                                
                                @foreach($article->tags as $tag)
                                <li><a href="#" class="tag-base tag-light-blue" style="background: #e3f2fd; color: #1976d2; margin-right: 0;">{{ $tag->name }}</a></li>
                                @endforeach
                                
                                <li style="width: 100%; display: flex; gap: 15px; align-items: center; margin-top: 10px;">
                                    <span><i class="fa fa-user"></i> <a href="{{ route('author.show', $article->user->username ?? Str::slug($article->user->name)) }}" class="text-secondary">{{ $article->user->name }}</a></span>
                                    <span><i class="fa fa-clock-o"></i> {{ $article->published_at->translatedFormat('d M Y') }}</span>
                                    <span><i class="fa fa-eye"></i> {{ $article->views }} {{ __('frontend.views') }}</span>
                                </li>
                            </ul>
                        </div>
