    ...
    <div class="container-fluid pt-4 px-4">
        <div class="row g-4">
            ...
            <div class="col-sm-6 col-xl-3">
                <div
                    class="bg-secondary rounded p-4 d-flex align-items-center justify-content-between h-100 position-relative overflow-hidden group-hover">
                    ...
                    <div class="icon-box bg-dark rounded-circle d-flex align-items-center justify-content-center"
                                    ...
                                    @foreach($upcoming_posts as $post)
                                        <tr>
                                            ...
                                            <td><span class="badge bg-warning text-dark"><i
                                                        class="fa fa-clock me-1"></i>Scheduled</span></td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
                            ...
                            <tbody>
                                @forelse($latest_articles as $article)
                                    <tr>
                                        ...
                                        <td><span
                                                class="badge bg-dark border border-secondary">{{ $article->category->name ?? 'None' }}</span>
                                        </td>
            ...
            <div class="col-lg-4">
                ...
                <div class="bg-secondary rounded p-4 mb-4">
                    ...
                    <div class="d-grid gap-2">
                        <a href="{{ route('admin.articles.create') }}" class="btn btn-primary text-start"><i
                                class="fa fa-plus-circle me-2"></i> Write New Article</a>
                        @if(auth()->user()->role === 1)
                            <a href="{{ route('admin.users.create') }}" class="btn btn-outline-light text-start"><i
                                    class="fa fa-user-plus me-2"></i> Add New User</a>
                            <a href="{{ route('admin.settings.index') }}" class="btn btn-outline-light text-start"><i
                                    class="fa fa-cog me-2"></i> Site Settings</a>
                        @endif
                    </div>
                </div>
                            ...
                            <div class="w-100">
                                <div class="d-flex justify-content-between">
                                    ...
                                    <small class="text-muted"
                                        style="font-size: 0.7rem;">{{ $comment->created_at->diffForHumans() }}</small>
                                </div>
                ...
                <div class="bg-secondary rounded p-4">
                    <div class="d-flex align-items-center justify-content-between mb-4">
                        ...
                        <button class="btn btn-sm btn-primary rounded-circle" onclick="addTask()"><i
                                class="fa fa-plus"></i></button>
                    </div>
                    <div class="d-flex mb-3">
                        <input class="form-control bg-dark border-0 form-control-sm" type="text" id="taskInput"
                            placeholder="Add task...">
                    </div>
                    <div id="taskList" style="max-height: 200px; overflow-y: auto;">
                        @foreach($tasks as $task)
                            <div class="d-flex align-items-center border-bottom border-dark py-2 task-item"
                                id="task-{{ $task->id }}">
                                <input class="form-check-input m-0" type="checkbox"
                                    onchange="toggleTask({{ $task->id }}, this.checked)" {{ $task->completed ? 'checked' : '' }}>
                                <span
                                    class="ms-3 {{ $task->completed ? 'text-decoration-line-through text-muted' : '' }} task-title small">{{ $task->title }}</span>
                                <button class="btn btn-link text-danger ms-auto p-0" onclick="deleteTask({{ $task->id }})"><i
