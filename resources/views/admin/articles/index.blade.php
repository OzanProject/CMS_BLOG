                ...
                <table class="table text-start align-middle table-bordered table-hover mb-0 text-nowrap">
                    ...
                    <tbody>
                        @forelse($articles as $index => $article)
                            <tr>
                                <td>
                                    <input class="form-check-input article-checkbox" type="checkbox" name="ids[]"
                                        value="{{ $article->id }}">
                                </td>
                                ...
                                <td>
                                    <span
                                        class="badge bg-dark border border-secondary">{{ $article->category->name ?? 'Uncategorized' }}</span>
                                </td>
                                <td>
                                    <span
                                        class="badge {{ $article->status === 'published' ? 'bg-success' : ($article->status === 'draft' ? 'bg-warning' : 'bg-danger') }}">
                                ...
                                <td>
                                    ...
                                    @if(auth()->id() == $article->user_id || auth()->user()->role == 1)
                                        <a href="#"
                                            onclick="event.preventDefault(); if(confirm('Are you sure?')) document.getElementById('destroy-{{ $article->id }}').submit();"
