            ...
            <div class="d-flex align-items-center justify-content-between mb-4 flex-wrap gap-2">
                ...
                <div class="btn-group" role="group">
                    <a href="{{ route('admin.comments.index', ['status' => 'all']) }}"
                        ...
                        {{ __('dashboard.show_all') }} <span
                            class="badge bg-white text-dark ms-1">{{ $counts['all'] }}</span>
                    </a>
                    <a href="{{ route('admin.comments.index', ['status' => 'pending']) }}"
                        class="btn btn-sm {{ $status == 'pending' ? 'btn-warning' : 'btn-outline-warning' }}">
                        {{ __('comments.pending') }} <span
                            class="badge bg-white text-dark ms-1">{{ $counts['pending'] }}</span>
                    </a>
                    <a href="{{ route('admin.comments.index', ['status' => 'approved']) }}"
                        class="btn btn-sm {{ $status == 'approved' ? 'btn-success' : 'btn-outline-success' }}">
                        {{ __('comments.approved') }} <span
                            class="badge bg-white text-dark ms-1">{{ $counts['approved'] }}</span>
                    </a>
                    <a href="{{ route('admin.comments.index', ['status' => 'spam']) }}"
                        class="btn btn-sm {{ $status == 'spam' ? 'btn-danger' : 'btn-outline-danger' }}">
                                        ...
                                        @if($comment->status !== 'approved')
                                            <form action="{{ route('admin.comments.update', $comment->id) }}" method="POST">
                                                ...
                                                <button type="submit" class="btn btn-sm btn-success"
                                                    title="{{ __('users.approve') ?? 'Approve' }}">
