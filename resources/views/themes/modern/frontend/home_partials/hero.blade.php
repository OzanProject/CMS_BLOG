    ...
    <section class="mb-16 mt-0">
        <div
            ...
            <div class="lg:col-span-7 flex flex-col items-start gap-6">
                <div
                    class="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-secondary-container/10 text-secondary border border-secondary/20">
                ...
                <h1
                    class="font-h1 text-h1 text-on-surface leading-[1.1] tracking-tight hover:text-secondary transition-colors duration-300">
                ...
                <a href="{{ route('article.show', $heroArticle->slug) }}"
                    class="mt-4 inline-flex items-center gap-2 bg-primary text-white px-8 py-4 rounded-xl font-label-caps text-sm uppercase tracking-widest hover:bg-secondary hover:shadow-premium-hover transition-all duration-300 group">
                    Read Full Story
                    <span
            ...
            <div class="lg:col-span-5 relative group">
                <div
                    class="absolute -inset-4 bg-secondary/5 rounded-[2rem] blur-2xl group-hover:bg-secondary/10 transition-all duration-500">
                </div>
                <div
                    class="relative h-[350px] lg:h-[450px] rounded-[1.5rem] overflow-hidden shadow-2xl bg-surface-container-high border border-white/20">
                    @if($heroArticle->featured_image)
                        {{-- [UPDATE DISINI]: Penambahan fetchpriority dan loading eager --}}
                        <img alt="{{ $heroArticle->title }}" fetchpriority="high" loading="eager"
                            class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700"
                            src="{{ asset('storage/' . $heroArticle->featured_image) }}">
                    @else
                        <div
                            class="w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-surface-container-high to-surface-container text-outline">
