...
<head>
    <meta charset="utf-8">
    <title>{{ $title ?? 'Welcome' }} - {{ $settings['site_name'] ?? config('app.name', 'Laravel') }}</title>
    ...
    @if(isset($settings['site_favicon']))
        <link href="{{ asset('storage/' . $settings['site_favicon']) }}" rel="icon">
    @else
                            ...
                            <a href="{{ url('/') }}" class="">
                                <h3 class="text-primary d-flex align-items-center">
                                    @if(isset($settings['site_logo']))
                                        <img src="{{ asset('storage/' . $settings['site_logo']) }}" alt="Logo" style="width: 40px; height: 40px; object-fit: cover;" class="rounded-circle me-2">
                                    @else
